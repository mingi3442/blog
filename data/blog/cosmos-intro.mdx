...
draft: false
summary: 'AppChain이란 무엇이며, AppChain 생계 중 대표적인 Cosmos EcoSystem은 어떤 생계일까 ?'
...
그렇다면 AppChain 생계 중 대표적인 Cosmos EcoSystem에 대해서도 궁금하다..!
...
Cosmos 네트워크는 Cosmos Hub, Tendermint 합의 알고리즘 및 IBC(Inter-Blockchain Communication) 프로토콜을 사용하여 블록체인이 안전하게 통신할 수 있도록 하여, 상호 연결된 앱과 서비스의 계속 확장되는 생계입니다.
...
_체인 A에서 토큰을 전송하기 위해 IBC 전송이 시작되면 `✪Token`을 Bonding 상로 잠금 처리_ ![](https://v1.cosmos.network/images/intro/05-ibc-01.svg)
...
Cosmos SDK를 이용해서 만든 Cosmos Ecosystem에 소속된 독립된 블록체인을 의미하며 각 Zone은 개별적인 상 머신과 합의 알고리즘을 갖고, 특정한 사용 사례나 애플리케이션에 맞게 최적화할 수 있습니다.
